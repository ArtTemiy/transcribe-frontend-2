...
export const AlertProvider: React.FC<AlertProviderProps> = ({ 
    ...
    const showError = useCallback((message: string, options?: Omit<AlertConfig, 'message' | 'variant'>) => {
        return showAlert({ ...options, message, variant: 'error' });
    }, [showAlert]);

    const showWarning = useCallback((message: string, options?: Omit<AlertConfig, 'message' | 'variant'>) => {
        return showAlert({ ...options, message, variant: 'warning' });
    }, [showAlert]);

    const showSuccess = useCallback((message: string, options?: Omit<AlertConfig, 'message' | 'variant'>) => {
        return showAlert({ ...options, message, variant: 'success' });
    }, [showAlert]);

    const showInfo = useCallback((message: string, options?: Omit<AlertConfig, 'message' | 'variant'>) => {
